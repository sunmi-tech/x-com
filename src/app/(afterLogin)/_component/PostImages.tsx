...
export default function PostImages({post}: Props) {
  ...
  if (post.Images.length === 2) {
    return (
      <div
        className={clsx(style.postImageSection, style.twoImage)}
      >
        <Link
          href={`/${post.User.id}/status/${post.postId}/photo/${post.Images[0].imageId}`}
  ...
  if (post.Images.length === 3) {
    return (
      <div
        className={clsx(style.postImageSection, style.threeImage)}
      >
        <Link
          href={`/${post.User.id}/status/${post.postId}/photo/${post.Images[0].imageId}`}
          style={{ backgroundImage: `url(${post.Images[0]?.link})`, backgroundSize: 'cover'}}>
  ...
  if (post.Images.length === 4) {
    return (
      <div
        className={clsx(style.postImageSection, style.fourImage)}
      >
        <Link
          href={`/${post.User.id}/status/${post.postId}/photo/${post.Images[0].imageId}`}
          style={{ backgroundImage: `url(${post.Images[0]?.link})`, backgroundSize: 'cover'}}>
